...
// @downloadURL  https://raw.githubusercontent.com/Ascendcorp/userscripts/main/dist/bitbucket/bitbucket.user.js
// @updateURL    https://raw.githubusercontent.com/Ascendcorp/userscripts/main/dist/bitbucket/bitbucket.user.js
// @grant        none
// ==/UserScript==
...
;(() => {
  ...
  const getBranchName = async () => {
    ...
    const branchElement =
      section?.lastElementChild?.firstElementChild?.children[2].firstElementChild?.firstElementChild?.firstElementChild
        ?.lastElementChild
  ...
  const markDeleteBranchAfterMerge = async () => {
    const deleteBranchCheckbox = await waitForElement<HTMLInputElement>(deleteBranchCheckboxSelector)
